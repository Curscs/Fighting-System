...
function ModelWrapper.new(Model)
	...
	for _, Descendant in next, Model:GetDescendants() do
		if Descendant:IsA("BasePart") then
			...
			Cache[Descendant] = {Matrix, Size, Color, Transparency, Material, CanCollide}
		end
	end
	self.Cache = Cache
...
function ModelWrapper:SetCollisions(DesiredState)
	for BasePart in next, self.Cache do
		BasePart.CanCollide = DesiredState
	end
end

function ModelWrapper:ResetCollisions()
	for BasePart, Data in next, self.Cache do
		BasePart.CanCollide = Data[6]
	end
end
